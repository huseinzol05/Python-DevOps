...
class Crawl(luigi.Task):
    ...
    def run(self):
        results = google_news_run(self.issue, limit=self.limit,
        year_start=self.year_start, year_end=self.year_end, debug=False, sleep_time_every_ten_articles=10)
...
class Sentiment(luigi.Task):
    ...
    def requires(self):
        return {'Crawl':Crawl(issue=self.issue,
...
class Save_to_Elastic(luigi.Task):
    ...
    def requires(self):
        return {'Sentiment':Sentiment(issue=self.issue,
        year_start=self.year_start,
        year_end=self.year_end,
        limit=self.limit)}
    ...
    def run(self):
        ...
        for index in range(0,len(sentiment),self.batch_size):
            batch=sentiment[index:min(index+self.batch_size,len(sentiment))]
            actions = [
            {"_index": self.index, "_type": "news",
            "_id": batch[j]['url'],
            "_source": batch[j]
            }
